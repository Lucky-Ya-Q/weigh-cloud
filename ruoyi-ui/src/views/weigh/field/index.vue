...
<!--      </el-col>-->
    ...
    <el-table v-loading="loading" :data="fieldList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="字段代码" align="center" prop="code" />
...
<script>
import { listField, getField, delField, addField, updateField } from "@/api/weigh/field";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除扩展字段编号为"' + ids + '"的数据项？').then(function() {
        return delField(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('weigh/field/export', {
        ...this.queryParams
